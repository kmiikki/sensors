...
The measurement interval is specified with the -s argument, whose unit is seconds. The amount of data to be stored in the memory can be specified with the -d argument, the unit of which is a day. The program limits the number of measurement lines that can be stored in the memory to 60 d measurements at an one-second interval. The limit can be calculated as follows: 3600 * 24 * 60 = 5,184,000 (= rows_limit). The space consumption of two sensors is then 356 MB. If (3600 * 24 * retention_time) / interval > rows_limit, a new retention time is calculated. When the rows_limit value is reached, new measurement data is stored in the memory using the FIFO principle.
...
Next, the program moves to the measurement loop, where it tries to read the measurement values of each sensor. If this fails, the relays will cut off the VCC and GND circuits of the sensors, wait 0.1 s, after which all relays will be returned to NC mode (also the simulation relay). After the circuits are closed, another 0.1 s wait time is required, after which an attempt is made to read the sensor values again. If the number of sensor restarts exceeds the value of the trials variable (the default value is 3), the program gives up trials for the next measurement interval. The measurement results are written to the log file and displayed on the screen if the last measurement attempt was successful. Failed NaN results are also logged if the program is started with the -nan argument. All error events are always logged in the error log itself, regardless of whether the -nan argument is selected or not.

Part of the log data (datetime decimal part of a second and timestamp in seconds) is removed with the re.sub() function before the result is written to the terminal. All data is written to the CSV file.

In the last step, before performing a new measurement, the disable_halt variable is set to False, i.e. the termination of the program is now possible. After that, the expected time of the next measurement is calculated using the formula: wait_time = count * interval - (tp1 – tp0). If wait_time > 0, the program waits the calculated time before continuing. Next, the measurement counter is increased by one, the disable_halt variable is set to True and a new measurement cycle can begin.
