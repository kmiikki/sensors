...
# -*- coding: utf-8 -*-
"""
Created on Mon May 20 02:13:53 2024

@author: Kim

Example, how to monitor new data:
$ watch -n 5 ./thp-analrh.py -n -ni
...
def generate_stats(names, data, precisions):    
    ...
    if a.is_integer() :
        ...
        a_str =str(a)
    out.append('')
    out.append('R.H.%: '+rh_str + '±' + ci_str)
    out.append('R.H.%: ['+a_str+'% CI: '+ci_low_str+'-'+ci_high_str+']')    
    return out
...
print('THP R.H.% data analyzer - (C) Kim Miikki 2024')
...
parser.add_argument('-c', action = 'store_true', help='create combined humidity graph for 2 sensors', required=False)
...
if len(df.columns) == 7:
    sensors = 1
    ys = np.array(df.iloc[:,5])
elif len(df.columns) == 10:
    sensors = 2
    ys = np.array(df.iloc[:,5])
    ys = np.vstack((ys, np.array(df.iloc[:,8])))
else:
...
# Calculate slope and statistics for R.H.% series
i = 0
results = []
while i < sensors:
...
names = ['Sensor', 'Slope', 'Constant', 'Time', 'Delta R.H.%',
...
if create_diff_graph:
    ...
    plt.grid()      
    fig.tight_layout()
    fname = 'humidity-range-diff-(s2-s1)'
    fname += '.png'
    file_path = Path(os.path.join(adir, fname))
    plt.savefig(file_path,dpi=300,bbox_inches='tight')
    plt.ticklabel_format(style='plain')
