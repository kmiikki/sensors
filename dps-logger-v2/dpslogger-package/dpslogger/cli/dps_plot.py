...
def validate_pressure_unit(df: pd.DataFrame, csv_path: Path) -> str:
    if "unit" not in df.columns:
        raise ValueError(f"{csv_path.name}: CSV must contain column 'unit'")
    ...
    if len(units) == 0:
        raise ValueError(f"{csv_path.name}: No valid pressure unit found in column 'unit'")
...
def load_csv(csv_path: Path) -> pd.DataFrame:
    ...
    unit = validate_pressure_unit(df, csv_path)
    df.attrs["pressure_unit"] = unit

    return df


def get_pressure_label(df: pd.DataFrame) -> str:
    return f"Pressure ({get_pressure_unit(df)})"


def get_pressure_unit(df: pd.DataFrame) -> str:
    return str(df.attrs.get("pressure_unit", "pressure_unit"))
...
def plot_hist(
    ...
    # Keep histogram bins exactly as given by hist_spec,
    # but show only a limited number of x-axis labels.
    ...
    if len(ticks) >= 2:
        tick_step = abs(float(ticks[1] - ticks[0]))
    else:
        tick_step = 1.0

    if tick_step > 0:
        decimals = max(0, int(math.ceil(-math.log10(tick_step))))
    else:
        decimals = 3
