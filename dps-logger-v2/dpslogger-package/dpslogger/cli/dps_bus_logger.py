...
def write_run_metadata(
    ...
    session_id: str,
    file_suffix: str | None,
    sensor_meta: Dict[int, dict[str, object]],
) -> Path:
...
def _read_sample(
    ...
    try:
        ...
        row = {
            ...
            "t_rel": f"{t_rel:.6f}",
            "addr": addr,
            "pressure": format_pressure_for_unit(float(pressure), unit),
            "unit": unit,
        ...
        with transport.opened():
            ...
            for addr, dps in dps_map.items():
                ...
                try:
                    ...
                    meta["identify_error"] = str(exc)

                try:
                    unit = dps.query_unit()
                    units_by_addr[addr] = unit
                    meta["unit"] = unit
                except Exception as exc:
                    fallback_unit = str(dps.cfg.unit)
                    units_by_addr[addr] = fallback_unit
                    meta["unit"] = fallback_unit
                    meta["unit_error"] = str(exc)

                sensor_meta[addr] = meta

                parts = [f"UNIT: {meta['unit']}"]
                if "identify" in meta:
                    parts.append(f"IDENT: {meta['identify']}")
                elif "identify_error" in meta:
                    parts.append(f"IDENT_ERR: {meta['identify_error']}")

                print(f"ADDR {addr:02d} " + " ".join(parts))
