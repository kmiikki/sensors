...
check_source_tree() {
    ...
    [[ -f "${SOURCE_DIR}/cli/dps_profile_cycle.py" ]] || die "Missing dps_profile_cycle.py"
    [[ -f "${SOURCE_DIR}/cli/dps_profile_to_config.py" ]] || die "Missing dps_profile_to_config.py"
...
install_wrappers() {
    ...
    write_module_wrapper "${BIN_DIR}/dps-profile-cycle" "dpslogger.cli.dps_profile_cycle"
    write_module_wrapper "${BIN_DIR}/dps-profile-to-config" "dpslogger.cli.dps_profile_to_config"
...
set_permissions() {
    ...
    chmod 755 "${BIN_DIR}/dps-profile-cycle"
    chmod 755 "${BIN_DIR}/dps-profile-to-config"
}
...
post_install_test() {
    ...
    "${BIN_DIR}/dps-profile-cycle" --help >/dev/null
    "${BIN_DIR}/dps-profile-to-config" --help >/dev/null
}
