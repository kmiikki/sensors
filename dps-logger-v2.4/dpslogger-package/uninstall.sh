...
This removes only the installed application under ${INSTALL_DIR}.
...
remove_wrappers() {
    ...
    remove_wrapper "${BIN_DIR}/dps-bus-logger"
    ...
    remove_wrapper "${BIN_DIR}/dps-autoread-off"
    remove_wrapper "${BIN_DIR}/dps-profile-cycle"
    remove_wrapper "${BIN_DIR}/dps-profile-to-config"
}
...
Not removed:
  ...
  - User-created CSV, PNG, JSON, and stats files outside ${INSTALL_DIR}
EOF
}
